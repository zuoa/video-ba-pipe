...
function runtimeProfile(runtime?: FaceRuntimeStatus): RuntimeProfile | undefined {
  ...
  if (backend === 'rknn') {
    return {
      ...
      storedRuntime: 'rknn',
      platformLabel,
      backendLabel: tr("RKNNLite · NPU 加速"),
      backendNote: tr("当前设备将直接使用 RKNN 制品。以后可在同一模型包中追加 ONNX 等其他平台制品。"),
      architecture,
      device: 'rk3588',
      accept: '.rknn',
  ...
  if (backend === 'tensorrt') {
    return {
      ...
      platformLabel,
      backendLabel: tr("TensorRT · GPU 加速"),
      backendNote: tr("上传 ONNX，由 TensorRT Execution Provider 在当前 NVIDIA 设备上执行。"),
      architecture,
  ...
  if (backend === 'onnxruntime-cuda' || backend === 'onnxruntime') {
    ...
    return {
      ...
      platformLabel,
      backendLabel: cuda ? tr("ONNX Runtime · CUDA 加速") : 'ONNX Runtime · CPU',
      backendNote: cuda
        ? tr("“onnxruntime”是模型格式运行时；当前实际使用 CUDAExecutionProvider，并非 CPU 推理。")
        : tr("同一份通用 ONNX 制品可在安装了 CUDA Provider 的设备上自动切换为 GPU 加速。"),
      architecture: 'any',
      device: 'any',
      accept: '.onnx',
      extensionLabel: 'ONNX（.onnx）',
  ...
  return {
    ...
    platformLabel,
    backendLabel: capabilities.torch_cuda_available ? tr("TorchScript · CUDA 加速") : 'TorchScript · CPU',
    backendNote: tr("上传由 torch.jit 导出的模型；模型本身应避免写死导出设备。"),
    architecture,
    device: 'any',
    accept: '.pt,.pth',
...
const FaceModelSetupWizard: React.FC<FaceModelSetupWizardProps> = ({
  ...
  const ensureFileExtension = (file: UploadFile | undefined, role: string) => {
    ...
    if (!extensions.some((extension) => file.name.toLowerCase().endsWith(extension))) {
      throw new Error(trf("__VAR0__文件必须是 __VAR1__", [role, profile.extensionLabel]));
    }
  };
  ...
  const goNext = async () => {
    ...
    if (step === 0) {
      if (!profile) {
        setSubmitError(tr("当前服务没有检测到可用的人脸推理运行时，请先检查运行环境。"));
        return;
      }
      try {
        if (sourceMode === 'package') {
          const file = packageFiles[0];
          if (!file) throw new Error(tr("请先下载并选择 Buffalo L ZIP 模型包。"));
          if (!file.name.toLowerCase().endsWith('.zip')) {
            throw new Error(tr("官方整包必须是 ZIP 文件，请不要手动解压。"));
          }
        } else {
          if (!detectionFiles.length || !embeddingFiles.length) {
            throw new Error(tr("请同时选择人脸检测模型和特征提取模型。"));
          }
  ...
  const artifactData = (
    ...
    if (!profile) throw new Error(tr("当前平台没有可用推理运行时"));
  ...
  const submit = async () => {
    ...
    if (
      ...
      setStep(0);
      setSubmitError(tr("模型文件已失效，请重新选择后再试。"));
      return;
    }
    ...
    try {
      ...
      if (!bundleId) {
        try {
          const response = await createFaceModelBundle({
            ...
            input_size: '112x112',
            license_name: sourceMode === 'package' ? 'InsightFace model zoo' : tr("用户上传"),
      ...
      if (sourceMode === 'package') {
        ...
        const uploadErrors = uploadResults
          .map((result, index) => result.status === 'rejected'
            ? `${index === 0 ? tr("检测模型") : tr("特征模型")}：${errorText(result.reason, tr("上传失败"))}`
            : undefined)
          .filter(Boolean);
      ...
      try {
        ...
        if (!currentBundle?.ready) {
          throw new Error(currentBundle?.error || tr("服务尚未识别到完整的检测/特征制品组合"));
        }
      ...
      setSubmitError(errorText(error, tr("模型配置失败，请检查文件后重试。")));
    } finally {
      setSubmitting(false);
    }
  };
    ...
    return (
      <Card className="face-model-wizard__upload" bordered={false}>
        <div className="face-model-wizard__role">
          ...
          <div>
            <strong>{detection ? tr("人脸检测模型") : tr("特征提取模型")}</strong>
            <small>{detection ? tr("定位人脸框与五个关键点") : trf("输出 __VAR0__ 维人脸特征", [form.getFieldValue('embedding_dimension') || 512])}</small>
          </div>
          <Tag color={files.length ? 'success' : 'default'}>{files.length ? tr("已选择") : tr("必需")}</Tag>
        </div>
        <Upload.Dragger
  ...
  const stepContent = () => {
        ...
        <>
          {profile ? (
            ...
            <Alert
              type="error"
              showIcon
              message={tr("没有检测到可用推理运行时")}
              description={tr("请检查 ONNX Runtime、CUDA、RKNNLite 或 Torch 的安装状态后刷新页面。")}
            />
          )}
          <div className="face-model-wizard__source-head">
            <div>
              <strong>{tr("选择模型来源")}</strong>
              <small>{tr("第一次配置建议使用官方整包，系统会自动识别两个模型。")}</small>
            </div>
            <Radio.Group
              value={sourceMode}
              optionType="button"
            ...
            >
              <Radio.Button value="package" disabled={!packageSupported}>{tr("官方整包")}</Radio.Button>
              <Radio.Button value="custom">{tr("分别上传")}</Radio.Button>
            </Radio.Group>
          </div>

          <div className="face-model-wizard__download-list" aria-label={tr("模型下载地址")}>
            <a
            ...
            >
              <FileZipOutlined />
              <span>
                <strong>{tr("Buffalo L 官方整包")}</strong>
                <small>{tr("检测 + 特征，可直接上传 ZIP")}</small>
              </span>
              <em><DownloadOutlined /> {tr("直接下载")}</em>
            </a>
            <a
            ...
            >
              <DownloadOutlined />
              <span>
                <strong>{tr("LVFace-B 高精度特征模型")}</strong>
                <small>{tr("可选；在“分别上传”中替换特征模型")}</small>
              </span>
              <em>{tr("直接下载")}</em>
            </a>
          </div>
          ...
          {sourceMode === 'package' ? (
            <div className="face-model-wizard__package-flow">
              <Alert
                ...
                type="success"
                showIcon
                message={tr("推荐：InsightFace Buffalo L（SCRFD-10G + ResNet50）")}
                description={tr("点击下载 ZIP，下载完成后不必解压，直接拖到下方即可。系统会自动提取 det_10g.onnx 和 w600k_r50.onnx。")}
              />
              <Card className="face-model-wizard__package-upload" bordered={false}>
                <div className="face-model-wizard__role">
                  <span>02</span>
                  <div>
                    <strong>{tr("上传刚下载的 Buffalo L 整包")}</strong>
                    <small>{tr("保持 ZIP 原样，系统会安全解包并校验检测/特征模型是否齐全")}</small>
                  </div>
                  <Tag color={packageFiles.length ? 'success' : 'default'}>
                    {packageFiles.length ? tr("已选择") : tr("必需")}
                  </Tag>
                </div>
                <Upload.Dragger
                ...
                >
                  <p className="ant-upload-drag-icon"><InboxOutlined /></p>
                  <p className="face-model-wizard__drop-title">{tr("拖入 buffalo_l.zip，或点击选择")}</p>
                  <p className="face-model-wizard__drop-hint">{tr("ZIP 模型整包 · 无需手动解压")}</p>
                </Upload.Dragger>
                {packageFiles[0] ? (
            ...
            <>
              <Alert
                ...
                type="info"
                showIcon
                message={tr("高级方式：分别上传检测模型和特征模型")}
                description={profile?.storedRuntime === 'rknn'
                  ? tr("当前是 RKNN 平台，请上传已经转换并验证过的两个 RKNN 制品。")
                  : tr("可使用解压后的 det_10g.onnx 作为检测模型，并用 w600k_r50.onnx 或上方 LVFace-B 作为特征模型。")}
              />
              <Row gutter={[16, 16]}>
              ...
              <div className="face-model-wizard__contract-hint">
                <SafetyCertificateOutlined />
                <span>
                  {tr("已支持 SCRFD 官方多尺度原始输出；特征模型固定接收 112×112 对齐人脸并输出 512 维特征。")}
                </span>
              </div>
            </>
          )}
        ...
        <Form form={form} layout="vertical" requiredMark="optional">
          {repairBundle ? (
            <Alert
              ...
              type="info"
              showIcon
              message={trf("继续配置 __VAR0__ · __VAR1__", [repairBundle.name, repairBundle.version])}
              description={tr("向导会沿用已有模型契约，并为当前平台补齐或替换一组检测/特征制品。")}
            />
          ) : (
            <Row gutter={16}>
              <Col xs={24} md={15}>
                <Form.Item
                  name="name"
                  label={tr("模型名称")}
                  rules={[{ required: true, message: tr("请输入便于识别的模型名称") }]}
                  extra={tr("已根据文件名自动填写，可以改成团队熟悉的名称。")}
                >
                  <Input maxLength={100} placeholder={tr("例如：园区人脸识别模型")} />
                </Form.Item>
              </Col>
              <Col xs={24} md={9}>
                <Form.Item name="version" label={tr("版本")} rules={[{ required: true, message: tr("请输入版本") }]}>
          ...
          <Row gutter={16}>
            <Col xs={24} md={12}>
              <Form.Item
                ...
                label={tr("特征维度")}
                rules={[{ required: true, message: tr("请输入特征模型输出维度") }]}
                extra={tr("Buffalo L、LVFace-B 等主流人脸特征模型均为 512。")}
              >
              ...
              children: (
                <>
                  <Form.Item
                    name="contract_id"
                    label={tr("特征契约标识")}
                    rules={[{ required: true, message: tr("请输入特征契约标识") }]}
                    extra={tr("同一契约下的各平台特征必须可直接互相比对。")}
                  >
                  ...
                  <Row gutter={16}>
                    <Col xs={24} md={12}>
                      <Form.Item
                        ...
                        label={tr("检测输入尺寸")}
                        rules={[{ pattern: /^\d+x\d+$/i, message: tr("请输入如 640x640 的尺寸") }]}
                      >
                        <Input placeholder="640x640" disabled={sourceMode === 'package'} />
                      </Form.Item>
                    </Col>
                    <Col xs={24} md={12}>
                      <Form.Item name="detection_output_format" label={tr("检测输出形式")}>
                        <Select disabled={sourceMode === 'package'} options={[
                          { value: 'scrfd', label: tr("SCRFD 官方多尺度输出（推荐）") },
                          { value: 'combined', label: tr("已解码单个数组 N×15") },
                          { value: 'separate', label: tr("框 / 分数 / 关键点三个数组") },
                        ]} />
                      </Form.Item>
                    </Col>
                  </Row>
                  <Form.Item name="coordinates_are_absolute" label={tr("检测坐标已是绝对像素")} valuePropName="checked">
                    <Switch />
                  </Form.Item>
                </>
              ),
    ...
    if (completed) {
      return (
        <Result
          status="success"
          title={tr("当前平台的人脸模型已配置完成")}
          subTitle={tr("检测模型与特征模型已成对上传并通过平台组合检查。下一步可以创建人脸库并绑定这个模型包。")}
    ...
    return (
      <div className="face-model-wizard__review">
        <Descriptions bordered size="small" column={1}>
          <Descriptions.Item label={tr("模型包")}>{values.name} · {values.version}</Descriptions.Item>
          <Descriptions.Item label={tr("当前平台")}>{profile?.platformLabel}</Descriptions.Item>
          <Descriptions.Item label={tr("推理方式")}>{profile?.backendLabel}</Descriptions.Item>
          <Descriptions.Item label={tr("模型来源")}>
            {sourceMode === 'package' ? trf("官方整包 · __VAR0__", [packageFiles[0]?.name]) : tr("分别上传")}
          </Descriptions.Item>
          <Descriptions.Item label={tr("检测模型")}>
            {sourceMode === 'package' ? tr("自动提取 det_10g.onnx") : detectionFiles[0]?.name}
          </Descriptions.Item>
          <Descriptions.Item label={tr("特征模型")}>
            {sourceMode === 'package' ? tr("自动提取 w600k_r50.onnx") : embeddingFiles[0]?.name} · {values.embedding_dimension} {tr("维")}
          </Descriptions.Item>
        </Descriptions>
        <Alert
          className="face-model-wizard__notice"
          type="info"
          showIcon
          message={sourceMode === 'package' ? tr("将自动导入并检查两个模型") : tr("将并行上传并检查两个模型")}
          description={tr("完成后会立即验证当前平台是否具备可用的检测/特征制品组合。")}
        />
        <Card className="face-model-wizard__progress" bordered={false}>
          <div className="face-model-wizard__progress-head">
            <strong>{submitting ? tr("正在配置，请勿关闭窗口") : completedTasks ? tr("配置进度") : tr("准备创建并检查")}</strong>
            <Progress percent={progressPercent} size="small" showInfo={false} />
          </div>
          {[
            ['bundle', repairBundle ? tr("沿用逻辑模型包") : tr("创建逻辑模型包")],
            ['detection', sourceMode === 'package' ? tr("从整包导入人脸检测模型") : tr("上传人脸检测模型")],
            ['embedding', sourceMode === 'package' ? tr("从整包导入特征提取模型") : tr("上传特征提取模型")],
            ['verify', tr("检查当前平台制品组合")],
          ].map(([key, label]) => (
    ...
    : [
        ...
        step < 2 ? (
          ...
          <AppButton key="submit" type="primary" tone="info" loading={submitting} onClick={() => void submit()}>
            {createdBundleId && submitError ? tr("重试配置") : repairBundle ? tr("补齐并检查") : tr("创建并检查")}
          </AppButton>
        ),
      ];
  ...
  return (
    <Modal
      ...
      title={repairBundle ? tr("继续配置人脸模型") : tr("快速配置人脸模型")}
      open={open}
      width={900}
      footer={footer}
      ...
      {submitError ? (
        <Alert
          ...
          description={createdBundleId
            ? trf("__VAR0__。已成功的内容会保留，你可以更换文件后直接重试。", [submitError])
            : submitError}
          aria-live="assertive"
        />
      ) : null}
