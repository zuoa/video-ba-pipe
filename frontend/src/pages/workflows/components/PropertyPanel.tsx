  ...
  return (
    <div className="property-panel">
      <div className="panel-header">
        <Space size="small">
          <SettingOutlined />
          <span className="panel-title">{tr("连线属性")}</span>
        </Space>
        <Button
          type="text"
          size="small"
        ...
        {isConditionSource ? (
          ...
          >
            <Form.Item
              label={tr("触发条件")}
              name="condition"
              extra={tr("「总是」只存在于编辑器，保存为 JSON null")}
            >
              <Select>
                <Option value="always">{tr("总是")}</Option>
                <Option value="detected">{tr("检测到")}</Option>
                <Option value="not_detected">{tr("未检测到")}</Option>
              </Select>
            </Form.Item>
          </Form>
        )}
...
const PropertyPanel: React.FC<PropertyPanelProps> = ({
  ...
  useEffect(() => {
    if (node) {
      ...
      if (nodeType === 'videoSource' || nodeType === 'source') {
        ...
        // The backend treats the Chinese default as a sentinel, while the form
        // should show that default in the language selected by the user.
        formValues.alertMessage = node.data.alertMessage === '检测到目标' || !node.data.alertMessage
          ? tr('检测到目标')
          : node.data.alertMessage;
  ...
  const handleUpdate = async (version: number) => {
    try {
      ...
      if (nodeType === 'algorithm') {
        ...
        updatedData.alertMessage = values.alertMessage === tr('检测到目标')
          ? '检测到目标'
          : values.alertMessage;
        ...
        if (vlConfigReady && values.vlValidationEnable === true) {
          ...
          if (!promptTemplate) {
            form.setFields([{
              ...
              errors: [tr("启用 VL 核验时必须配置核验提示词模板")],
            }]);
            return;
          }
        }
  ...
  const runHttpRequestTest = async () => {
    ...
    try {
      ...
      message.success(result?.result?.success ? tr("测试请求成功") : tr("测试请求已完成，请查看失败详情"));
    } catch (error: any) {
      if (requestIsCurrent() && !error?.errorFields) {
        message.error(error?.data?.error || error?.message || tr("测试请求失败"));
      }
    } finally {
      if (requestIsCurrent()) {
        setHttpTesting(false);
      ...
      case 'source':
        if (isTemplate) {
          return (
            <div className="info-box template-source-info">
              <InfoCircleOutlined />
              <div>
                <Text strong>{tr("视频源占位节点")}</Text>
                <div>{tr("模板无需选择具体视频源，复制为普通编排时自动绑定。")}</div>
              </div>
            </div>
          );
        }
        ...
        return (
          <>
                ...
                <div className="current-source-card">
                  ...
                  <div className="source-card-details">
                    <div className="detail-item">
                      <span className="detail-label">{tr("编码:")}</span>
                    ...
                    {currentSource.decoder_type && (
                      <div className="detail-item">
                        <span className="detail-label">{tr("解码器:")}</span>
            ...
            <Form.Item
              name="videoSourceId"
              rules={[{ required: true, message: tr("请选择视频源") }]}
              hidden
            >
              <Input />
            </Form.Item>
            ...
            {videoSources.length === 0 && (
              <div className="info-box">
                <InfoCircleOutlined />
                <span>{tr("暂无可用视频源，请先在视频源管理中添加")}</span>
              </div>
            )}
          ...
          <>
            {isVlAlgorithm ? (
              <div className="info-box" style={{ marginBottom: 16, background: '#f0fffe', borderColor: '#87e8de', color: '#006d75' }}>
                <InfoCircleOutlined />
                <span>{tr("此节点调用 VL 语义算法；提示词和模型参数在算法管理中统一维护。")}</span>
              </div>
            ) : null}
            {isOcrAlgorithm ? (
              <div className="info-box" style={{ marginBottom: 16, background: '#f0f7ff', borderColor: '#91caff', color: '#0958d9' }}>
                <InfoCircleOutlined />
                <span>{tr("此节点调用本地 OCR；检测与识别模型在算法管理中统一维护。")}</span>
              </div>
            ) : null}
            <Form.Item
              label={isOcrAlgorithm ? tr("文字置信度阈值") : tr("置信度阈值")}
              name="confidence"
            >
              <Select>
                <Option value={0.3}>{tr("0.3 (低)")}</Option>
                <Option value={0.5}>{tr("0.5 (中)")}</Option>
                <Option value={0.7}>{tr("0.7 (高)")}</Option>
                <Option value={0.9}>{tr("0.9 (极高)")}</Option>
              </Select>
            </Form.Item>
            ...
            <div className="config-section">
              <div className="config-section-header">
                <span className="config-section-title">{tr("执行配置")}</span>
              </div>

              <Form.Item
                label={tr("检测间隔（秒）")}
                name="intervalSeconds"
                extra={tr("每N秒执行一次检测，1表示每帧都检测")}
              >
                      ...
                      <>
                        {hasAlgorithmIncoming && inputMode === 'frame' ? (
                          <div className="info-box" style={{ marginBottom: 16, background: '#e6f4ff', borderColor: '#91caff', color: '#0958d9' }}>
                            <InfoCircleOutlined />
                            <span>{tr("若要只识别检测框内文字，请把连线设为『检测到』，并把 OCR 输入模式改为『上游裁剪』。")}</span>
                          </div>
                        ) : null}
                        <Form.Item
                          label={tr("输入模式")}
                          name="inputMode"
                          extra={tr("整帧识别或仅对上游检测框裁剪识别")}
                        >
                          <Select>
                            <Option value="frame">{tr("整帧")}</Option>
                            <Option value="upstream_crops">{tr("上游裁剪")}</Option>
                          </Select>
                        </Form.Item>
                        <Form.Item
                          label={tr("扩边比例")}
                          name="expandRatio"
                          extra={tr("每边按框尺寸扩边，与组合检测一致")}
                        >
                        ...
                        <Form.Item
                          ...
                          extra={tr("留空不过滤；匹配 class_name / label / label_name")}
                        >
                          <Select mode="tags" tokenSeparators={[',', '，']} disabled={cropDisabled} placeholder={tr("全部类别")} />
                        </Form.Item>
                      </>
                    );
                  }}
              ...
              {isVlAlgorithm ? (
                <>
                  <Form.Item
                    ...
                    extra={trf("关闭时使用算法配置中的 __VAR0__ 秒", [vlDefaultTimeout])}
                  >
                    <Switch />
                  </Form.Item>
                  <Form.Item noStyle shouldUpdate={(previous, current) => previous.vlTimeoutOverrideEnabled !== current.vlTimeoutOverrideEnabled}>
                    {({ getFieldValue }) => getFieldValue('vlTimeoutOverrideEnabled') ? (
                      <Form.Item
                        label={tr("节点调用超时（秒）")}
                        name="runtimeTimeout"
                        rules={[{ required: true, message: tr("请输入节点调用超时") }]}
                        extra={tr("仅覆盖当前 workflow 节点")}
                      >
              ...
              <Form.Item
                label={tr("标签名称")}
                name="labelName"
                extra={<>{tr("支持")} <code>{'{class}'}</code> {tr("占位符，将替换为实际识别类别")}</>}
              >
            ...
            {isTrackerEventAlgorithm ? (
              <>
                ...
                <div className="config-section">
                  <div className="config-section-header">
                    <span className="config-section-title">{tr("输出事件")}</span>
                  </div>
                  <Form.Item
                    label={tr("事件类型")}
                    name="trackEvent"
                    extra={tr("在追踪结果上直接判定，不必再串徘徊/停留节点")}
                  >
                    <Select>
                      <Option value="none">{tr("全部轨迹（只赋 ID）")}</Option>
                      <Option value="loiter">{tr("徘徊（区域内待够久）")}</Option>
                      <Option value="stay">{tr("停留（原地待够久）")}</Option>
                      <Option value="region_cross">{tr("穿越热区（按方向）")}</Option>
                    </Select>
                  </Form.Item>
                    ...
                    {({ getFieldValue }) => {
                      ...
                      if (event === 'loiter' || event === 'stay') {
                        return (
                          <>
                            <Form.Item
                              label={event === 'stay' ? tr("停留多久（秒）") : tr("徘徊多久（秒）")}
                              name="minDwellSeconds"
                              extra={tr("同一 ID 持续出现达到该时长才输出")}
                            >
                            ...
                            <Form.Item
                              label={tr("最小位移（像素）")}
                              name="minDisplacePx"
                              extra={tr("活动半径低于该值不算。徘徊可用来排除站着不动；0 表示不限制")}
                            >
                            ...
                            <Form.Item
                              label={tr("最大位移（像素）")}
                              name="maxDisplacePx"
                              extra={event === 'stay' ? tr("超出该半径视为在移动，不填默认 48") : tr("超出该半径不算徘徊；留空不限制")}
                            >
                              <InputNumber min={0} max={4000} step={1} style={{ width: '100%' }} placeholder={event === 'stay' ? '48' : tr("不限制")} />
                            </Form.Item>
                            <Form.Item
                              label={tr("消失后遗忘（秒）")}
                              name="disappearSeconds"
                              extra={tr("目标丢失超过该时间后重新计时")}
                            >
                      ...
                      if (event === 'region_cross') {
                        return (
                          <>
                            <Form.Item label={tr("穿越方式")} name="crossMode" extra={tr("热区来自上游 ROI 绘制节点")}>
                              <Select>
                                <Option value="enter">{tr("进入热区")}</Option>
                                <Option value="exit">{tr("离开热区")}</Option>
                                <Option value="cross">{tr("进入或离开")}</Option>
                              </Select>
                            </Form.Item>
                            <Form.Item label={tr("穿越方向")} name="crossDirection">
                              <Select>
                                <Option value="any">{tr("任意方向")}</Option>
                                <Option value="left_to_right">{tr("从左到右")}</Option>
                                <Option value="right_to_left">{tr("从右到左")}</Option>
                                <Option value="top_to_bottom">{tr("从上到下")}</Option>
                                <Option value="bottom_to_top">{tr("从下到上")}</Option>
                              </Select>
                            </Form.Item>
                            <Form.Item
                              label={tr("消失后遗忘（秒）")}
                              name="disappearSeconds"
                              extra={tr("目标丢失超过该时间后重新检测穿越")}
                            >
                              <InputNumber min={0.5} max={120} step={0.5} style={{ width: '100%' }} />
                            </Form.Item>
                          </>
        ...
        return (
          <>
            <Form.Item
              label={tr("选择 API")}
              name="externalApiId"
              rules={[{ required: true, message: tr("请选择外部 API") }]}
            >
              <Select placeholder={tr("选择已配置的外部 API")}>
            ...
            {externalApis.length === 0 && (
              <div className="info-box" style={{ marginBottom: 16 }}>
                <InfoCircleOutlined />
                <span>{tr("暂无可用外部 API，请先在“外部 API”页面中配置")}</span>
              </div>
            )}
            ...
            <div className="config-section">
              ...
              <Form.Item
                label={tr("执行模式")}
                name="executionMode"
                extra={tr("同步等待会阻塞当前节点；异步提交只投递请求，不等待结果")}
              >
                <Select>
                  <Option value="sync">{tr("同步等待")}</Option>
                  <Option value="async_submit">{tr("异步提交")}</Option>
                </Select>
              </Form.Item>
              ...
              <Form.Item label={tr("包含图片")} name="includeImage" valuePropName="checked">
                <Switch />
              </Form.Item>

              <Form.Item label={tr("包含上游结果")} name="includeUpstreamResults" valuePropName="checked">
                <Switch />
              </Form.Item>
            </div>
            ...
            <div className="config-section">
              <div className="config-section-header">
                <span className="config-section-title">{tr("输入参数")}</span>
              </div>
              <div className="info-box" style={{ marginBottom: 12 }}>
                <InfoCircleOutlined />
                <span>{tr("系统会自动注入 `workflow_id`、`node_id`、`frame_timestamp`，可选注入 `image_base64` 和 `upstream_results`。")}</span>
              </div>
              <Form.Item
                label={tr("附加请求体")}
                name="payloadTemplate"
                extra={tr("JSON 对象，会与系统自动生成的字段合并")}
              >
            ...
            <div className="config-section">
              <div className="config-section-header">
                <span className="config-section-title">{tr("输出参数")}</span>
              </div>
              <DetectionResponseContract />
              <Form.Item
                label={tr("输出映射")}
                name="outputMapping"
                extra={tr("请用点路径声明返回值位置。支持 has_detection_path / detections_path / metadata_path / label_color_path")}
              >
        ...
        return (
          <>
            <div className="info-box" style={{ marginBottom: 16, background: '#f0fbfb', borderColor: '#87e8de', color: '#006d75' }}>
              <InfoCircleOutlined />
              <span>{tr("使用")} {'{{ $.source.code }}'} {tr("引用系统字段，或用")} {'{{ $.nodes["node-id"].outputs.name }}'} {tr("引用上游结果。")}</span>
            </div>

            <div className="config-section">
              <div className="config-section-header"><span className="config-section-title">{tr("请求")}</span></div>
              <Form.Item label={tr("请求方法")} name="httpMethod" rules={[{ required: true }]}>
              ...
              <Form.Item
                ...
                name="httpUrl"
                rules={[
                  { required: true, message: tr("请输入 HTTP/HTTPS URL") },
                  { pattern: /^(https?:\/\/|\s*\{\{\s*\$)/i, message: tr("URL 必须以 http://、https:// 或完整 JSONPath 模板开头") },
                ]}
              >
                <Input placeholder="https://api.example.com/status" />
              </Form.Item>
              <Space size={12} style={{ width: '100%' }}>
                <Form.Item label={tr("调用间隔（秒）")} name="httpIntervalSeconds" rules={[{ required: true }]} style={{ flex: 1 }}>
                  <InputNumber min={0.1} max={300} step={0.1} style={{ width: '100%' }} />
                </Form.Item>
                <Form.Item label={tr("超时（秒）")} name="httpTimeoutSeconds" rules={[{ required: true }]} style={{ flex: 1 }}>
                  <InputNumber min={1} max={300} style={{ width: '100%' }} />
                </Form.Item>
              </Space>
            </div>
            ...
            <div className="form-divider" />
            <div className="config-section">
              <div className="config-section-header"><span className="config-section-title">{tr("Query 参数")}</span></div>
                  ...
                  <div className="http-config-list">
                    {fields.map(({ key, name: fieldName, ...restField }) => (
                      <div className="http-config-row" key={key}>
                        ...
                        <Form.Item {...restField} name={[fieldName, 'name']} rules={[{ required: true, message: tr("名称不能为空") }]}>
                          <Input placeholder={tr("参数名")} />
                        </Form.Item>
                        <Form.Item {...restField} name={[fieldName, 'value']}>
                          <Input.Password placeholder={tr("值或模板")} visibilityToggle={false} />
                        </Form.Item>
                        <Form.Item {...restField} name={[fieldName, 'sensitive']} valuePropName="checked">
                          <Switch size="small" checkedChildren={tr("敏感")} unCheckedChildren={tr("普通")} />
                        </Form.Item>
                        <Button type="text" danger icon={<MinusCircleOutlined />} onClick={() => remove(fieldName)} aria-label={tr("删除 Query 参数")} />
                      </div>
                    ))}
                    <Button block type="dashed" icon={<PlusOutlined />} onClick={() => add({ enabled: true, name: '', value: '', sensitive: false })}>{tr("添加 Query 参数")}</Button>
                  </div>
                )}
              </Form.List>
            </div>
            ...
            <div className="form-divider" />
            <div className="config-section">
              <div className="config-section-header"><span className="config-section-title">{tr("请求头")}</span></div>
                  ...
                  <div className="http-config-list">
                    {fields.map(({ key, name: fieldName, ...restField }) => (
                      <div className="http-config-row" key={key}>
                        ...
                        <Form.Item {...restField} name={[fieldName, 'value']}>
                          <Input.Password placeholder={tr("值或模板；留空保持敏感值")} visibilityToggle={false} />
                        </Form.Item>
                        <Form.Item {...restField} name={[fieldName, 'sensitive']} valuePropName="checked">
                          <Switch size="small" checkedChildren={tr("敏感")} unCheckedChildren={tr("普通")} />
                        </Form.Item>
                        <Button type="text" danger icon={<MinusCircleOutlined />} onClick={() => remove(fieldName)} aria-label={tr("删除请求头")} />
                      </div>
                    ))}
                    <Button block type="dashed" icon={<PlusOutlined />} onClick={() => add({ enabled: true, name: '', value: '', sensitive: false })}>{tr("添加请求头")}</Button>
                  </div>
                )}
              </Form.List>
                      ...
                      validator: (_, value) => {
                        try {
                          ...
                          return parsed === null || Array.isArray(parsed) || typeof parsed === 'object'
                            ? Promise.resolve()
                            : Promise.reject(new Error(tr("请求体必须是 JSON 对象、数组或 null")));
                        } catch {
                          return Promise.reject(new Error(tr("JSON 请求体格式无效")));
                        }
                      },
                    }]}
                  >
            ...
            <div className="form-divider" />
            <div className="config-section">
              <div className="config-section-header"><span className="config-section-title">{tr("JSONPath 输出")}</span></div>
              <Form.List name="httpExtractors">
                {(fields, { add, remove }) => (
                  <div className="http-config-list">
                    {fields.map(({ key, name: fieldName, ...restField }) => (
                      <div className="http-config-row extractor-row" key={key}>
                        <Form.Item {...restField} name={[fieldName, 'name']} rules={[{ required: true }, { pattern: /^[A-Za-z_][A-Za-z0-9_]*$/, message: tr("仅支持字母、数字和下划线，且不能以数字开头") }]}>
                          <Input placeholder="risk_score" />
                        </Form.Item>
                        <Form.Item {...restField} name={[fieldName, 'jsonpath']} rules={[{ required: true }, { pattern: /^\$/, message: tr("JSONPath 必须以 $ 开头") }]}>
                          <Input placeholder="$.data.score" />
                        </Form.Item>
                        <Form.Item {...restField} name={[fieldName, 'required']} valuePropName="checked">
                          <Switch size="small" checkedChildren={tr("必填")} unCheckedChildren={tr("可选")} />
                        </Form.Item>
                        <Button type="text" danger icon={<MinusCircleOutlined />} onClick={() => remove(fieldName)} aria-label={tr("删除提取规则")} />
                      </div>
                    ))}
                    <Button block type="dashed" icon={<PlusOutlined />} onClick={() => add({ name: '', jsonpath: '$.', required: false })}>{tr("添加提取变量")}</Button>
                  </div>
                )}
              </Form.List>
            </div>
            ...
            <div className="config-section http-test-console">
              <div className="config-section-header"><span className="config-section-title">{tr("测试请求")}</span></div>
              <Form.Item label={tr("模拟上下文 JSON")} name="httpTestContext">
                <TextArea rows={5} />
              </Form.Item>
      ...
      case 'webhook': {
        ...
        const validateJson = (expected: 'object' | 'array') => (_: any, value: string) => {
          try {
            ...
            return valid
              ? Promise.resolve()
              : Promise.reject(new Error(expected === 'array' ? tr("必须是 JSON 数组") : tr("必须是 JSON 对象")));
          } catch {
            return Promise.reject(new Error(tr("JSON 格式无效")));
          }
        };
        ...
        return (
          <>
            <div className="info-box" style={{ marginBottom: 16 }}>
              <InfoCircleOutlined />
              <span>{tr("仅在上游告警真正落库后异步推送；工作流测试只生成预览，不访问外部端点。")}</span>
            </div>

            <Form.Item label={tr("协议")} name="webhookProvider">
              <Select>
                <Option value="generic">{tr("通用 JSON Webhook")}</Option>
                <Option value="dingtalk">{tr("钉钉自定义机器人")}</Option>
            ...
            <Form.Item
              ...
              rules={[{
                validator: (_, value) => value || (
                  ...
                  ? Promise.resolve()
                  : Promise.reject(new Error(tr("请输入 Webhook 端点地址"))),
              }]}
              extra={endpointConfigured && !webhookConfig.endpoint_url
                ? trf("已配置：__VAR0__；留空保持原值", [webhookConfig.endpoint_display || '******'])
                : tr("仅支持 HTTP/HTTPS；钉钉必须使用官方机器人地址")}
            >
              <Input.Password
                placeholder={endpointConfigured ? tr("留空保持已配置地址") : 'https://...'}
                ...
                if (provider === 'dingtalk') {
                  return (
                    <Form.Item
                      ...
                      extra={signingSecretConfigured ? tr("已配置；留空保持原值") : tr("可选，对应钉钉机器人 SEC 密钥")}
                    >
                      <Input.Password
                        placeholder={signingSecretConfigured ? tr("留空保持原值") : 'SEC...'}
                        onBlur={() => void commitWebhookCredentials()}
                      />
                    </Form.Item>
                  );
                ...
                if (provider === 'bark') {
                  return (
                    <>
                      <Form.Item
                        ...
                        rules={[{
                          validator: (_, value) => value || barkKeyConfigured
                            ? Promise.resolve()
                            : Promise.reject(new Error(tr("请输入 Bark Device Key"))),
                        }]}
                        extra={barkKeyConfigured ? tr("已配置；留空保持原值") : undefined}
                      >
                        <Input.Password
                          placeholder={barkKeyConfigured ? tr("留空保持原值") : 'Bark Device Key'}
                          onBlur={() => void commitWebhookCredentials()}
                        />
                      </Form.Item>
                      <Form.Item label={tr("通知分组")} name="webhookBarkGroup">
                        <Input placeholder="VideoBA" />
                      </Form.Item>
                      <Form.Item label={tr("中断级别")} name="webhookBarkLevel">
                        <Select>
                      ...
                      <Form.Item label={tr("提示音")} name="webhookBarkSound">
                        <Input placeholder={tr("可选，例如 alarm")} />
                      </Form.Item>
                    </>
                  );
                }
            ...
            <Form.Item label={tr("标题模板")} name="webhookTitleTemplate">
              <Input placeholder="【{{alert.level}}】{{alert.type}}" />
            </Form.Item>
            <Form.Item
              label={tr("正文模板")}
              name="webhookBodyTemplate"
              extra={tr("支持 {{alert.message}}、{{source.name}}、{{detection.detections}} 等点路径占位符")}
            >
              <TextArea rows={4} />
            </Form.Item>
            <Form.Item label={tr("附带媒体 URL")} name="webhookIncludeMediaUrls" valuePropName="checked">
              <Switch />
            </Form.Item>
            <Form.Item
              label={tr("媒体公共基地址")}
              name="webhookPublicBaseUrl"
              extra={tr("可选节点级覆盖；留空时继承系统设置中的全局公共访问地址")}
            >
            ...
            <div className="config-section">
              <div className="config-section-header">
                <span className="config-section-title">{tr("投递策略")}</span>
              </div>
              <Form.Item label={tr("单次超时（秒）")} name="webhookTimeoutSeconds">
                <InputNumber min={1} max={30} style={{ width: '100%' }} />
              </Form.Item>
              <Form.Item label={tr("最多尝试次数")} name="webhookMaxAttempts">
                <InputNumber min={1} max={5} style={{ width: '100%' }} />
              </Form.Item>
              <Form.Item label={tr("初始退避（秒）")} name="webhookRetryBackoffSeconds">
            ...
            <div className="config-section">
              <div className="config-section-header">
                <span className="config-section-title">{tr("高级请求配置")}</span>
              </div>
              <Form.Item
                label={tr("请求头")}
                name="webhookHeaders"
                rules={[{ validator: validateJson('array') }]}
                extra={tr("JSON 数组：[{\"name\":\"Authorization\",\"value\":\"Bearer ...\",\"sensitive\":true}]")}
              >
                <TextArea rows={6} />
              </Form.Item>
              <Form.Item noStyle shouldUpdate={(previous, current) => previous.webhookProvider !== current.webhookProvider}>
                {({ getFieldValue }) => getFieldValue('webhookProvider') === 'generic' ? (
                  <Form.Item
                    ...
                    extra={tr("留空对象时发送完整标准事件；支持模板占位符")}
                  >
      ...
      case 'condition': {
        ...
        return (
          <>
            <div className="info-box" style={{ marginBottom: 16 }}>
              <InfoCircleOutlined />
              <span>{tr("条件节点根据检测结果、OCR 文字或 HTTP 命名变量控制 yes/no 分支")}</span>
            </div>
            ...
            >
              <Select>
                <Option value="count">{tr("目标数量")}</Option>
                <Option value="count_change">{tr("数量骤变")}</Option>
                <Option value="ocr_text">{tr("OCR 文字")}</Option>
                <Option value="http_value">{tr("API 值条件")}</Option>
              </Select>
            </Form.Item>
              ...
              {({ getFieldValue }) => getFieldValue('conditionKind') === 'http_value' ? (
                <>
                  <Form.Item
                    label={tr("HTTP 来源节点")}
                    name="sourceNodeId"
                    rules={[{ required: true, message: tr("请选择已连接的 HTTP 请求节点") }]}
                  >
                    <Select placeholder={tr("选择上游 HTTP 请求节点")}>
                  ...
                  {upstreamHttpNodes.length === 0 ? (
                    <div className="info-box" style={{ marginBottom: 16, background: '#fff7e6', borderColor: '#ffd591', color: '#ad4e00' }}>
                      <InfoCircleOutlined />
                      <span>{tr("请先将一个 HTTP 请求节点连接到当前条件节点。")}</span>
                    </div>
                  ) : null}
                  <Form.Item
                    label={tr("AND / OR 条件组")}
                    ...
                    rules={[{
                      validator: (_, value) => value?.children?.length
                        ? Promise.resolve()
                        : Promise.reject(new Error(tr("至少添加一条条件规则"))),
                    }]}
                  >
                  ...
                  <div className="info-box">
                    <InfoCircleOutlined />
                    <span>{tr("比较值按 JSON 强类型处理：数字 1 不等于字符串 \"1\"。内置变量可判断请求成功、状态码和错误类型。")}</span>
                  </div>
                </>
                ...
                <>
                  <Form.Item
                    label={tr("OCR 来源节点")}
                    name="sourceNodeId"
                    rules={[{ required: true, message: tr("请选择已连接的 OCR 节点") }]}
                  >
                    <Select placeholder={tr("选择上游 OCR 节点")}>
                  ...
                  {upstreamOcrNodes.length === 0 ? (
                    <div className="info-box" style={{ marginBottom: 16, background: '#fff7e6', borderColor: '#ffd591', color: '#ad4e00' }}>
                      <InfoCircleOutlined />
                      <span>{tr("请先将 OCR 算法节点连接到当前条件节点。")}</span>
                    </div>
                  ) : null}
                  <Form.Item label={tr("判断方式")} name="textOperator" initialValue="contains">
                    <Select>
                      <Option value="contains">{tr("包含")}</Option>
                      <Option value="not_contains">{tr("不包含")}</Option>
                    </Select>
                  </Form.Item>
                  <Form.Item label={tr("匹配方式")} name="patternType" initialValue="keywords">
                    <Select>
                      <Option value="keywords">{tr("关键词")}</Option>
                      <Option value="regex">{tr("正则表达式")}</Option>
                    </Select>
                  </Form.Item>
                  <Form.Item noStyle shouldUpdate={(previous, current) => previous.patternType !== current.patternType}>
                    {({ getFieldValue: getPatternType }) => getPatternType('patternType') === 'regex' ? (
                      <Form.Item
                        ...
                        name="regexPattern"
                        rules={[
                          { required: true, message: tr("请输入正则表达式") },
                          {
                            validator: (_, value) => {
                              ...
                              try {
                                ...
                                return Promise.reject(new Error(tr("正则表达式无效")));
                              }
                            },
                          },
                        ]}
                      >
                        <Input placeholder={tr("例如：车牌[A-Z0-9]{6}")} />
                      </Form.Item>
                    ) : (
                      <>
                        <Form.Item
                          label={tr("关键词")}
                          name="keywords"
                          rules={[{ required: true, type: 'array', min: 1, message: tr("至少输入一个关键词") }]}
                          extra={tr("输入关键词后按回车确认")}
                        >
                          <Select mode="tags" tokenSeparators={[',', '，']} placeholder={tr("输入关键词")} />
                        </Form.Item>
                        <Form.Item label={tr("关键词逻辑")} name="keywordLogic" initialValue="any">
                          <Select>
                            <Option value="any">{tr("任一关键词命中")}</Option>
                            <Option value="all">{tr("全部关键词命中")}</Option>
                          </Select>
                        </Form.Item>
                      </>
                    )}
                  </Form.Item>
                  <Form.Item label={tr("区分大小写")} name="caseSensitive" valuePropName="checked">
                    <Switch />
                  </Form.Item>
                </>
                ...
                <>
                  <Form.Item
                    label={tr("统计来源节点")}
                    name="sourceNodeId"
                    rules={[{ required: true, message: tr("请选择已连接的上游结果节点") }]}
                  >
                    <Select placeholder={tr("选择上游算法、函数或外部 API 节点")}>
                  ...
                  {upstreamResultNodes.length === 0 ? (
                    <div className="info-box" style={{ marginBottom: 16, background: '#fff7e6', borderColor: '#ffd591', color: '#ad4e00' }}>
                      <InfoCircleOutlined />
                      <span>{tr("请先连接一个算法、函数或外部 API 节点。")}</span>
                    </div>
                  ) : null}
                  <Form.Item
                    label={tr("目标类别")}
                    name="labels"
                    extra={tr("留空统计全部目标；输入类别后按回车确认，不区分大小写")}
                  >
                    <Select mode="tags" tokenSeparators={[',', '，']} placeholder={tr("全部类别")} />
                  </Form.Item>
                  <Form.Item
                    label={tr("历史窗口（次）")}
                    name="windowSize"
                    rules={[{ required: true }, { type: 'number', min: 2, max: 300 }]}
                    extra={tr("积满历史样本后，从下一次检测开始判断")}
                  >
                    <InputNumber min={2} max={300} step={1} style={{ width: '100%' }} />
                  </Form.Item>
                  <Form.Item label={tr("变化方向")} name="direction" rules={[{ required: true }]}>
                    <Select>
                      <Option value="both">{tr("骤增或骤减")}</Option>
                      <Option value="increase">{tr("仅骤增")}</Option>
                      <Option value="decrease">{tr("仅骤减")}</Option>
                    </Select>
                  </Form.Item>
                  <Form.Item
                    label={tr("相对变化阈值（%）")}
                  ...
                  <Form.Item
                    ...
                    extra={tr("触发后采用边沿语义，恢复相同次数后重新布防")}
                  >
                ...
                <>
                  <Form.Item label={tr("比较类型")} name="comparisonType">
                    <Select>
                      <Option value=">=">{tr("至少N个 (≥)")}</Option>
                      <Option value="==">{tr("正好N个 (=)")}</Option>
                    </Select>
                  </Form.Item>
                  <Form.Item
                    label={tr("数量阈值")}
                    name="targetCount"
                    rules={[
                      { required: true, message: tr("请输入数量阈值") },
                      { type: 'number', min: 1, max: 1000, message: tr("请输入1-1000之间的数字") }
                    ]}
                  >
        ...
        return (
          <>
            <div className="info-box" style={{ marginBottom: 16, background: '#f0f5ff', borderColor: '#adc6ff', color: '#1d39c4' }}>
              <InfoCircleOutlined />
              <span>{tr("按服务器本地时区判断。起止分钟均包含，跨日时段请拆成两天配置。")}</span>
            </div>
            <Form.Item
              label={tr("每周启用时段")}
              name="weeklySchedule"
                  ...
                  for (const { key, label } of WEEKDAYS) {
                    for (const period of schedule[key]) {
                      ...
                      if (!/^([01]\d|2[0-3]):[0-5]\d$/.test(period.start || '') || !/^([01]\d|2[0-3]):[0-5]\d$/.test(period.end || '')) {
                        return Promise.reject(new Error(trf("__VAR0__存在未填写或格式错误的时间", [tr(label)])));
                      }
                      if (period.start > period.end) {
                        return Promise.reject(new Error(trf("__VAR0__存在跨日时段，请拆成两天配置", [tr(label)])));
                      }
                    }
                  }
                  return periodCount > 0
                    ? Promise.resolve()
                    : Promise.reject(new Error(tr("至少需要配置一个启用时段")));
                },
              }]}
            >
              <TimeScheduleEditor />
      ...
      case 'detection_filter': {
        ...
        return (
          <>
            <div className="config-section">
              <div className="config-section-header">
                <span className="config-section-title">{tr("输入状态")}</span>
              </div>
            ...
            <div className="config-section">
              <div className="config-section-header">
                <span className="config-section-title">{tr("尺寸保留规则")}</span>
              </div>

              <Form.Item label={tr("检测维度")} name="filterDimension" rules={[{ required: true }]}>
                <Select>
                  <Option value="height">{tr("高度")}</Option>
                  <Option value="width">{tr("宽度")}</Option>
                </Select>
              </Form.Item>

              <Form.Item label={tr("尺寸单位")} name="filterUnit" rules={[{ required: true }]}>
                <Select>
                  <Option value="pixel">{tr("像素")}</Option>
                  <Option value="ratio">{tr("占画面比例")}</Option>
                </Select>
              </Form.Item>

              <Form.Item label={tr("保留条件")} name="filterComparison" rules={[{ required: true }]}>
                <Select>
                  <Option value="gte">{tr("大于等于（最小值）")}</Option>
                  <Option value="lte">{tr("小于等于（最大值）")}</Option>
                </Select>
              </Form.Item>
              ...
              <Form.Item noStyle shouldUpdate={(previous, current) => previous.filterUnit !== current.filterUnit}>
                {({ getFieldValue }) => {
                  ...
                  return (
                    <Form.Item
                      label={unit === 'ratio' ? tr("阈值（%）") : tr("阈值（px）")}
                      name="filterThreshold"
                      extra={unit === 'ratio'
                        ? tr("以目标高度/画面高度或目标宽度/画面宽度计算")
                        : tr("按检测框在原始画面中的像素尺寸计算")}
                      rules={[
                        { required: true, message: tr("请输入尺寸阈值") },
                        {
                          type: 'number',
                          min: 0,
                          max: unit === 'ratio' ? 100 : 10000000,
                          message: unit === 'ratio' ? tr("请输入 0–100 之间的比例") : tr("请输入非负像素值"),
                        },
                      ]}
                    >
                      <InputNumber
      ...
      case 'function':
        ...
        return (
          <>
            ...
            <div className="config-section">
              <div className="config-section-header">
                <span className="config-section-title">{tr("输入配置（自动识别）")}</span>
              </div>

              {upstreamNodes.length === 0 ? (
                <div className="info-box" style={{ background: '#fff7e6', borderColor: '#ffd591', color: '#d46b08' }}>
                  <InfoCircleOutlined />
                  <span>{tr("请先从上游算法节点连线到当前函数节点")}</span>
                </div>
              ) : (
                <>
                  <div className="info-box" style={{ background: '#f6ffed', borderColor: '#b7eb8f', color: '#52c41a', marginBottom: 12 }}>
                    <InfoCircleOutlined />
                    <span>{tr("已自动识别")} {upstreamNodes.length} {tr("个上游算法节点")}</span>
                  </div>
                    ...
                    return (
                      <div key={upstreamNode.id} style={{ marginBottom: 16 }}>
                        <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 8, color: '#262626' }}>
                          {tr("输入节点")}{letter}：{upstreamNode.data?.label || upstreamNode.id}
                        </div>
                        ...
                        <Form.Item
                          label={trf("类别过滤__VAR0__", [letter])}
                          name={index === 0 ? 'classFilterA' : 'classFilterB'}
                          style={{ marginBottom: 0 }}
                        >
                          <Input placeholder={tr("如: 0,1,2 (留空表示全部类别)")} />
                        </Form.Item>
                      ...
                      if (singleInputFunctions.includes(functionName) && upstreamNodes.length > 1) {
                        return (
                          <div className="info-box" style={{ marginTop: 12, background: '#fff7e6', borderColor: '#ffd591', color: '#d46b08' }}>
                            <InfoCircleOutlined />
                            <span>{tr("此函数为单输入模式，将只使用第一个输入节点")}</span>
                          </div>
                        );
                      }
                      return null;
                    ...
                    {({ getFieldValue }) => {
                      ...
                      if (!singleInputFunctions.includes(functionName) && upstreamNodes.length < 2) {
                        return (
                          <div className="info-box" style={{ marginTop: 12, background: '#fff7e6', borderColor: '#ffd591', color: '#d46b08' }}>
                            <InfoCircleOutlined />
                            <span>{tr("此函数需要2个输入节点，请再连接一个算法节点")}</span>
                          </div>
                        );
                      }
                      return null;
            ...
            <div className="config-section">
              <div className="config-section-header">
                <span className="config-section-title">{tr("计算函数与判定条件")}</span>
              </div>
              ...
              >
                <Select>
                  <Option value="area_ratio">{tr("面积比（双输入）")}</Option>
                  <Option value="height_ratio">{tr("高度比（双输入）")}</Option>
                  <Option value="width_ratio">{tr("宽度比（双输入）")}</Option>
                  <Option value="iou_check">{tr("IOU检查（双输入）")}</Option>
                  <Option value="distance_check">{tr("距离检查（双输入）")}</Option>
                  <Option value="height_ratio_frame">{tr("高度占图片比例（单输入）")}</Option>
                  <Option value="width_ratio_frame">{tr("宽度占图片比例（单输入）")}</Option>
                  <Option value="area_ratio_frame">{tr("面积占图片比例（单输入）")}</Option>
                  <Option value="size_absolute">{tr("绝对尺寸检测（单输入）")}</Option>
                </Select>
              </Form.Item>
              ...
              >
                <Select>
                  <Option value="less_than">{tr("小于")}</Option>
                  <Option value="greater_than">{tr("大于")}</Option>
                  <Option value="equal">{tr("等于")}</Option>
                </Select>
              </Form.Item>
                  ...
                  <Form.Item noStyle shouldUpdate={(prevValues, currentValues) => prevValues.functionName !== currentValues.functionName}>
                    {({ getFieldValue }) => {
                      ...
                      if (singleInputFunctions.includes(functionName)) {
                        return tr("对于比例函数，值为0-1之间；对于绝对尺寸函数，值为像素值");
                      }
                      return tr("值为0-1之间的小数，如0.7表示70%");
                    }}
                  </Form.Item>
                }
              >
                    ...
                    return (
                      ...
                      >
                        <Select>
                          <Option value="height">{tr("高度")}</Option>
                          <Option value="width">{tr("宽度")}</Option>
                          <Option value="area">{tr("面积")}</Option>
                        </Select>
                      </Form.Item>
                    );
                  }
      ...
      case 'roi':
        ...
        return (
          <>
            ...
            <div className="config-section">
              <div className="config-section-header">
                <span className="config-section-title">{tr("ROI 区域管理")}</span>
              </div>
              ...
              <div className="roi-status-box">
                <div className="roi-status-info">
                  ...
                  <span>
                    {roiRegions.length > 0
                      ? trf("已配置 __VAR0__ 个区域", [roiRegions.length])
                      : tr("未配置区域")}
                  </span>
                </div>
                <Button
                ...
                >
                  {roiRegions.length > 0 ? tr("编辑区域") : tr("绘制区域")}
                </Button>
              </div>
              ...
              {!roiVideoSource && (
                <div className="info-box" style={{ marginTop: 12 }}>
                  <InfoCircleOutlined />
                  <span>{tr("请确保工作流中有可用的视频源并正确连接")}</span>
                </div>
              )}
              ...
              {roiVideoSource && (
                <div className="info-box" style={{
                  ...
                  <InfoCircleOutlined />
                  <span>{tr("视频源:")} {roiVideoSource.name}</span>
                </div>
              )}

              {roiRegions.length > 0 && (
                <div className="roi-regions-list">
                  <Text strong>{tr("已配置的 ROI 区域:")}</Text>
                  <List
                    size="small"
          ...
          <>
            ...
            >
              <Select>
                ...
                <Option value="error">{tr("错误")}</Option>
            ...
            <Form.Item
              label={tr("告警类型")}
              name="alertType"
              extra={tr("用于区分不同类型的告警")}
            >
              <Input placeholder={tr("例如: person, vehicle, fire")} />
            </Form.Item>
            <Form.Item
              label={tr("告警消息")}
              name="alertMessage"
              extra={tr("自定义告警消息前缀，会自动追加执行详情")}
            >
              <Input placeholder={tr("自定义告警消息")} />
            </Form.Item>
            <Form.Item
              label={tr("消息格式")}
              name="messageFormat"
            ...
            >
              <Select
                placeholder={tr("请选择消息格式")}
                onChange={(value) => console.log('🔄 Select onChange:', value)}
              >
                <Option value="detailed">{tr("详细格式（包含节点ID）")}</Option>
                <Option value="simple">{tr("简单格式（仅消息内容）")}</Option>
                <Option value="summary">{tr("汇总格式（按级别分组）")}</Option>
              </Select>
            </Form.Item>
            ...
            <div className="config-section">
              <div className="config-section-header">
                <span className="config-section-title">{tr("触发条件配置（窗口检测）")}</span>
              </div>

              <Form.Item
                label={tr("启用窗口检测")}
                name="triggerConditionEnable"
                extra={tr("是否启用窗口检测验证，禁用时所有检测都会触发告警")}
                valuePropName="checked"
              >
                <Switch />
              </Form.Item>
                ...
                {({ getFieldValue }) => {
                  ...
                  if (!enableTrigger) {
                    return (
                      <div className="info-box" style={{ marginTop: 12, background: '#f6ffed', borderColor: '#b7eb8f', color: '#52c41a' }}>
                        <InfoCircleOutlined />
                        <span>{tr("窗口检测已禁用，所有检测都将触发告警")}</span>
                      </div>
                    );
                  }
                  ...
                  return (
                    <>
                      ...
                      >
                        <Select>
                          <Option value="count">{tr("检测次数 (count)")}</Option>
                          <Option value="ratio">{tr("检测比例 (ratio)")}</Option>
                          <Option value="consecutive">{tr("连续检测 (consecutive)")}</Option>
                        </Select>
                      </Form.Item>
                      ...
                      <Form.Item noStyle shouldUpdate={(prevValues, currentValues) => prevValues.triggerConditionMode !== currentValues.triggerConditionMode}>
                        {({ getFieldValue }) => {
                          ...
                          if (mode === 'ratio') {
                            label = tr("检测阈值（比例）");
                            extra = tr("0-1之间的小数，如0.3表示30%的帧检测到目标");
                            inputType = 'ratio';
                          } else if (mode === 'count') {
                            label = tr("检测阈值（次数）");
                            extra = tr("正整数，时间窗口内最少检测次数");
                            inputType = 'count';
                          } else if (mode === 'consecutive') {
                            label = tr("检测阈值（次数）");
                            extra = tr("正整数，最少连续检测次数");
                            inputType = 'count';
                          }
            ...
            <div className="config-section">
              <div className="config-section-header">
                <span className="config-section-title">{tr("VL 二次核验")}</span>
              </div>

              <Form.Item
                label={tr("启用 VL 核验")}
                name="vlValidationEnable"
                extra={tr("开启后，告警在真正输出前会调用全局配置的 VL 模型做一次复核。")}
                valuePropName="checked"
              >
              ...
              <Form.Item noStyle shouldUpdate={(prevValues, currentValues) => prevValues.vlValidationEnable !== currentValues.vlValidationEnable}>
                {({ getFieldValue }) => {
                  ...
                  return (
                    <div className="info-box" style={{ marginTop: 12, background: '#fff7e6', borderColor: '#ffd591', color: '#d46b08' }}>
                      <InfoCircleOutlined />
                      <span>{tr("全局未配置 VL 模型参数，当前节点不可启用 VL 二次核验")}</span>
                    </div>
                  );
                }
                  ...
                  return (
                    <div className="info-box" style={{ marginTop: 12, background: '#f6ffed', borderColor: '#b7eb8f', color: '#52c41a' }}>
                      <InfoCircleOutlined />
                      <span>{tr("VL 核验已禁用，满足条件后将直接输出告警")}</span>
                    </div>
                  );
                }
                      ...
                      rules={[
                        {
                          validator: (_, value) => {
                            ...
                            return Promise.reject(new Error(tr("启用 VL 核验时必须配置核验提示词模板")));
                          }
                        }
                      ]}
                      extra={tr("支持占位符：{alert_type}、{alert_message}、{detection_count}、{detection_summary}、{detections_json}、{workflow_name}、{workflow_id}、{node_id}")}
                    >
                      <Input.TextArea
                        rows={8}
                        placeholder={tr("例如：请根据图像判断是否真的发生“{alert_type}”告警。\n当前告警文案：{alert_message}\n检测数量：{detection_count}\n检测摘要：\n{detection_summary}\n\n只返回JSON：{\"is_alert\": true/false, \"confidence\": 0-1, \"reason\": \"...\"}")}
                      />
                    </Form.Item>

                    <div className="info-box" style={{ marginTop: 12, background: '#fff7e6', borderColor: '#ffd591', color: '#d46b08' }}>
                      <InfoCircleOutlined />
                      <span>{tr("每个告警输出节点都应单独配置核验提示词模板，用于表达当前算法的判定规则")}</span>
                    </div>
                    </>
                  );
                }}
            ...
            <div className="config-section">
              <div className="config-section-header">
                <span className="config-section-title">{tr("告警抑制配置（触发后冷却期）")}</span>
              </div>

              <Form.Item
                label={tr("启用告警抑制")}
                name="suppressionEnable"
                extra={tr("是否启用告警抑制，启用后触发告警的 N 秒内不会再触发")}
                valuePropName="checked"
              >
                <Switch />
              </Form.Item>
                ...
                {({ getFieldValue }) => {
                  ...
                  if (!enableSuppression) {
                    return (
                      <div className="info-box" style={{ marginTop: 12, background: '#f6ffed', borderColor: '#b7eb8f', color: '#52c41a' }}>
                        <InfoCircleOutlined />
                        <span>{tr("告警抑制已禁用，满足触发条件时每次都会告警")}</span>
                      </div>
                    );
                  }
            ...
            <div className="config-section">
              <div className="config-section-header">
                <span className="config-section-title">{tr("消息投递输出")}</span>
              </div>

              <Form.Item
                label={tr("输出到消息通道")}
                name="publishToMq"
                extra={tr("开启后，该节点告警在真正触发（通过触发条件、抑制期、VL 核验）后会异步投递到系统设置中选择的 MQTT、RabbitMQ 或 HTTP API。关闭后告警记录与录像不受影响。")}
                valuePropName="checked"
              >
                <Switch />
              </Form.Item>
      ...
      case 'record':
        return (
          <>
            ...
            >
              <Select>
                <Option value={5}>{tr("5 秒")}</Option>
                <Option value={10}>{tr("10 秒")}</Option>
                <Option value={30}>{tr("30 秒")}</Option>
                <Option value={60}>{tr("60 秒")}</Option>
              </Select>
            </Form.Item>
          </>
        );
  ...
  return (
    <div className="property-panel">
      <div className="panel-header">
        <Space size="small">
          <SettingOutlined />
          <span className="panel-title">{tr("节点属性")}</span>
        </Space>
        <Button
          type="text"
          size="small"
      ...
      >
        <Tabs.TabPane tab={tr("基本属性")} key="basic">
          <Form
            key={node.id}
            form={form}
            layout="vertical"
          ...
          >
            <Form.Item
              label={tr("节点名称")}
              name="label"
              rules={[{ required: true, message: tr("请输入节点名称") }]}
            >
              <Input size="small" />
            </Form.Item>
            ...
            <div className="auto-update-hint" role="status">
              <CheckCircleOutlined />
              <span>{tr("修改会自动应用到画布，点击顶部“保存”后生效")}</span>
            </div>
          </Form>
        </Tabs.TabPane>

        <Tabs.TabPane tab={tr("节点信息")} key="info">
          <div className="node-info">
            <div className="info-row">
              <span className="info-label">{tr("节点 ID:")}</span>
              <span className="info-value">{node.id}</span>
            </div>
            <div className="info-row">
              <span className="info-label">{tr("节点类型:")}</span>
              <Tag color={node.data.color}>{node.data.label}</Tag>
            </div>
            <div className="info-row">
              <span className="info-label">{tr("位置:")}</span>
