...
const STATUS_LABELS: Record<string, { text: string; color: string }> = {
  reuse: { text: tr("按标识复用"), color: 'success' },
  reuse_by_hash: { text: tr("按内容复用"), color: 'success' },
  mapped: { text: tr("已映射"), color: 'processing' },
  import: { text: tr("随包导入"), color: 'blue' },
  missing: { text: tr("需要映射"), color: 'warning' },
  conflict: { text: tr("名称冲突"), color: 'error' },
  unsupported: { text: tr("目标设备不支持"), color: 'error' },
};

async function readManifestFromPackage(file: File): Promise<TemplateTransferManifest> {
  if (file.size < 30) throw new Error(tr("迁移包内容不完整"));
  const header = new DataView(await file.slice(0, 30).arrayBuffer());
  if (header.getUint32(0, true) !== 0x04034b50) throw new Error(tr("不是有效的迁移 ZIP 包"));
  ...
  if (compression !== 0) throw new Error(tr("迁移包 manifest.json 必须使用未压缩格式"));
  if (compressedSize <= 0 || compressedSize > 1024 * 1024) throw new Error(tr("迁移清单大小无效"));
  const nameStart = 30;
  const dataStart = nameStart + filenameLength + extraLength;
  const filename = new TextDecoder().decode(
    await file.slice(nameStart, nameStart + filenameLength).arrayBuffer(),
  );
  if (filename !== 'manifest.json') throw new Error(tr("迁移包第一项不是 manifest.json"));
  const raw = await file.slice(dataStart, dataStart + compressedSize).text();
  const manifest = JSON.parse(raw);
  if (manifest?.format !== 'video-ba-workflow-template') throw new Error(tr("不是 Video BA 编排模板迁移包"));
  return manifest;
}

const statusTag = (status: string) => {
  const item = STATUS_LABELS[status] || { text: status || tr("待检查"), color: 'default' };
...
export default function TemplateTransferModal({
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setError(requestError?.data?.error || requestError?.message || tr("读取设备迁移能力失败"));
      }
    };
    load();
  }, [mode, open]);
    ...
    setResolutions((current) => {
      ...
      result.blockers.forEach((blocker) => {
        ...
        if (blocker.resource === 'template') {
          next.template = { action: 'rename', name: trf("__VAR0__（导入）", [result.template.name]) };
        } else if (blocker.resource === 'model' && blocker.included && portableId) {
          next.models![portableId] = { action: 'rename', name: trf("__VAR0__（导入）", [blocker.name]), version: blocker.version };
        } else if (blocker.resource === 'algorithm' && portableId) {
          next.algorithms![portableId] = { action: 'rename', name: trf("__VAR0__（导入）", [blocker.name]) };
        } else if (blocker.resource === 'external_api' && portableId) {
          next.external_apis![portableId] = { action: 'rename', name: trf("__VAR0__（导入）", [blocker.name]) };
        } else if (blocker.resource === 'hook' && portableId) {
          next.hooks![portableId] = { action: 'rename', name: trf("__VAR0__（导入）", [blocker.name]) };
        }
      });
      return next;
    });
  ...
  const handleFile = useCallback(async (nextFile: File) => {
    ...
    try {
      ...
      setManifest(null);
      setPreflight(null);
      setError(nextError?.data?.error || nextError?.message || tr("迁移包预检失败"));
    } finally {
      setBusy(false);
    }
    return false;
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      setError(nextError?.message || tr("导出失败"));
    } finally {
      setBusy(false);
    }
  };
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (!checked.ready && checked.template.status !== 'already_imported') {
        setError(tr("仍有未解决的模型映射、名称冲突或必填配置，请检查标红项目"));
        return;
      }
      ...
      setError(nextError?.data?.error || nextError?.message || tr("导入失败"));
    } finally {
      setBusy(false);
    }
  };
  ...
  const dependencyRows = preflight ? [
    ...preflight.dependencies.models.map((item) => ({ ...item, kind: tr("模型") })),
    ...preflight.dependencies.algorithms.map((item) => ({ ...item, kind: tr("算法") })),
    ...preflight.dependencies.external_apis.map((item) => ({ ...item, kind: tr("外部 API") })),
  ...
  const exportBody = (
    <div className="template-transfer">
      <div className="transfer-device-rail">
        <div>
          <span>{tr("当前盒子型号")}</span>
          <strong>{capabilities?.device_model_code || (error ? tr("读取失败") : tr("读取中…"))}</strong>
        </div>
        <SafetyCertificateOutlined />
      </div>
      {capabilities === null && !error ? (
        <div className="transfer-loading"><Spin /><span>{tr("正在读取设备信息…")}</span></div>
      ) : null}
      {capabilities && !capabilities.configured ? (
        <Alert type="error" showIcon message={tr("尚未配置设备型号")} description={tr("请先在部署环境中设置 DEVICE_MODEL_CODE，再重新启动服务。")} />
      ) : null}
      <Descriptions column={1} size="small" bordered>
        <Descriptions.Item label={tr("编排模板")}>{template?.name}</Descriptions.Item>
        <Descriptions.Item label={tr("运行依赖")}>{tr("算法、脚本、Hook、外部 API 与模型清单")}</Descriptions.Item>
      </Descriptions>
      <label className="transfer-model-option">
        <Checkbox checked={includeModels} onChange={(event) => setIncludeModels(event.target.checked)} />
        <span>
          <strong>{tr("携带模型文件")}</strong>
          <small>{tr("适合目标盒子离线或尚未部署模型的场景，迁移包可能较大。")}</small>
        </span>
      </label>
  ...
  const importBody = (
    <div className="template-transfer">
      <Upload.Dragger {...uploadProps} disabled={busy} className="transfer-dropzone">
        ...
        <p className="ant-upload-text">{tr("选择编排模板迁移包")}</p>
        <p className="ant-upload-hint">{tr("系统会先读取清单并核对盒子型号，不会立即上传大模型文件。")}</p>
      </Upload.Dragger>

      {busy && !preflight ? <div className="transfer-loading"><Spin /><span>{tr("正在读取迁移清单…")}</span></div> : null}
      {preflight ? (
        <>
          <div className="transfer-device-rail transfer-device-rail--pair">
            <div><span>{tr("导出设备")}</span><strong>{preflight.source.device_model_code}</strong></div>
            <SwapOutlined />
            <div><span>{tr("当前设备")}</span><strong>{preflight.target.device_model_code}</strong></div>
            <CheckCircleOutlined />
          </div>
          <div className="transfer-summary">
            <div><span>{tr("模板")}</span><strong>{preflight.template.name}</strong></div>
            <div><span>{tr("应用版本")}</span><strong>{preflight.source.app_version}</strong></div>
            <div><span>{tr("依赖项")}</span><strong>{dependencyRows.length}</strong></div>
          </div>
          ...
          ) : <Alert type="info" showIcon message={tr("此模板不依赖模型、算法或外部 API")} />}

          {preflight.blockers.length ? <Divider orientation="left">{tr("解决依赖与冲突")}</Divider> : null}
            ...
            if (blocker.status === 'unsupported') {
              return (
                <Alert
                  ...
                  type="error"
                  showIcon
                  message={trf("目标设备不支持“__VAR0__”所需的推理能力", [blocker.name])}
                  description={tr("请先在目标设备安装对应运行时，或使用已兼容的算法资源。")}
                />
              );
            }
          ...
          {preflight.required_inputs.length ? <Divider orientation="left">{tr("导入时必填")}</Divider> : null}
  ...
  return (
    <AppModal
      open={open}
      title={mode === 'export' ? tr("导出编排模板") : tr("导入编排模板")}
      description={mode === 'export' ? tr("生成可在同型号盒子间迁移的依赖包") : tr("先核对设备型号，再解决目标环境中的依赖")}
      size="lg"
      onCancel={onClose}
      keyboard={!busy}
      maskClosable={false}
      footer={(
        <Space>
          <Button onClick={onClose} disabled={busy}>{tr("取消")}</Button>
          <Button
            type="primary"
